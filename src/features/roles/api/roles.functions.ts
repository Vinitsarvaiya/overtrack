...
} from "@/features/roles/schemas/roles.schemas";
...
export const createRole = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { assertPermission, forbidden } = await import("@/features/roles/server/rbac.server");
...
export const renameRole = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { assertPermission, forbidden, workspaceOfRole } =
      await import("@/features/roles/server/rbac.server");
...
export const deleteRole = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const workspaceId = await workspaceOfRole(context.supabase, data.roleId);
    await assertPermission(context.supabase, workspaceId, context.userId, "settings.edit");
    const { error } = await context.supabase.from("workspace_roles").delete().eq("id", data.roleId);
    if (error) throw forbidden();
    return { ok: true };
  });
