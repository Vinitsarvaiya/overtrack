...
import {
  ...
  type TimeFormat,
  workedHours,
} from "@/features/entries/lib/overtime";
import { uploadAttachment } from "@/features/entries/lib/attachments";
import { useCalendarDays } from "@/features/calendar/hooks/use-calendar-days";
import { useEntries, useSaveEntry } from "@/features/entries/hooks/use-entries";
...
function composeTimeValue(hour: string, minute: string, period: "AM" | "PM", format: TimeFormat) {
...
export function EntryDialog({
  ...
  useEffect(() => {
    ...
    if (entry) {
      form.reset({
        ...
        notes: entry.notes ?? "",
        overtime_override: entry.overtime_override === null ? "" : String(entry.overtime_override),
      });
  ...
  async function submitEntry(raw: EntryFormValues, mode: "draft" | "submit") {
    ...
    if (
      !workspace.allow_future_dates &&
      parsed.entry_date > new Date().toISOString().slice(0, 10)
    ) {
