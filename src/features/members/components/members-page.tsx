...
import { MemberRateHistoryDrawer } from "@/features/members/components/member-rate-history-drawer";
import { useUpdateMemberRates, useWorkspaceMembers } from "@/features/members/hooks/use-members";
import { type Member } from "@/features/members/types";
import { formatRate } from "@/features/calendar/lib/calendar";
import { useMemberMutations } from "@/features/members/hooks/use-member-mutations";
import { useRoleMutations, useWorkspaceRoles } from "@/features/roles/hooks/use-roles";
import { MemberRole } from "@/features/members/lib/member-access";
import { allowedAssignableRoles } from "@/features/members/lib/member-access";
import { canManageTargetRole } from "@/features/members/lib/member-access";

import type { InviteRole } from "@/features/members/lib/member-access";
...
export function MembersPage() {
  ...
  const inviteableRoles = useMemo(
    () => allowedAssignableRoles(role, permissions),
    [role, permissions],
  );
  const currency = workspace?.currency ?? "USD";
  const [historyMember, setHistoryMember] = useState<
    (Member & { profile: { full_name: string | null; email: string | null } | null }) | null
  >(null);
  ...
  function removeMember(memberId: string) {
    ...
    remove.mutate(memberId, {
      ...
      onError: () =>
        toast.error("You do not have permission to remove this member", { id: toastId }),
    });
  }
        ...
        <Card>
          ...
          <CardContent className="flex flex-wrap gap-2">
            ...
            >
              ...
              <SelectContent>
                {inviteableRoles.includes("admin") ? (
