...
import { useWorkspaceMembers } from "@/features/members/hooks/use-members";
import { useWorkspace } from "@/features/workspaces/context/workspace-provider";
import { baseStandardHours } from "@/features/calendar/lib/working-calendar";
import {
  STATUS_LABELS,
  formatHours,
  overtimeHours,
...
export function DashboardPage() {
  ...
  const daily = useMemo(() => {
    ...
    entries
      ...
      .forEach((entry) => {
        map.set(
          entry.entry_date,
          (map.get(entry.entry_date) ?? 0) + overtimeHours(entry, standard),
        );
      });
    return [...map.entries()]
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <StatCard icon={Clock} label="Today's overtime" value={formatHours(stats.todayOvertime)} />
        <StatCard
          icon={TrendingUp}
          label="This month hours"
          value={formatHours(stats.monthHours)}
        />
        <StatCard icon={CalendarDays} label="Days worked this month" value={String(stats.days)} />
          ...
          {entries.slice(0, 6).map((entry) => (
            ...
            >
              ...
              <div className="flex items-center gap-4 text-muted-foreground">
                ...
                <span className="text-primary">+{formatHours(overtimeHours(entry, standard))}</span>
